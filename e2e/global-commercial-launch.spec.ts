...
test("public room catalogue resolves only valid rooms", async ({ page }) => {
  ...
  await expect(page.getByRole("main").getByText("OAV Musiguda Library")).toBeVisible();
...
test("room authentication and tenancy boundaries hold in the browser", async ({ page }) => {
  ...
  await expect(page.locator("p.notice-error[role='alert']")).toContainText("Unable to sign in");
...
test("a second room retains its independent USD localization", async ({ page }) => {
  await page.goto("/l/E2EROOM/login");
  await page.getByLabel("Email").fill("e2e-room-b-admin@granthsetu.invalid");
  await page.getByLabel("Password").fill(password);
  await page.getByRole("button", { name: "Sign in" }).click();
  await expect(page).toHaveURL(/\/operator\/E2EROOM/);
  await page.goto("/operator/E2EROOM/settings");
