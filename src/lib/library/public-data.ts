...
export type PublicShelfSummary = { code: string; name: string; totalCopies: number; availableCopies: number };
export type PublicShelf = { code: string; name: string; titleCount: number; copyCount: number; availableCopyCount: number };
...
export async function getPublicCataloguePage(rawCode: string, query = "", availableOnly = false, requestedPage = 1, pageSize = 24, shelfCode = ""): Promise<PublicCataloguePage | null> {
  const code = normalizeLibraryCode(rawCode);
  const safeQuery = query.trim().slice(0, 160);
  ...
  if (isLocalDemo(code)) {
    const needle = safeQuery.toLowerCase();
    const shelf = shelfCode.trim().toLowerCase(); const filtered = demoBooks.filter((book) => (!availableOnly || book.availableCopies > 0) && (!shelf || book.shelves.some((item) => item.code.toLowerCase() === shelf)) && (!needle || `${book.title} ${book.authorNames} ${book.isbn ?? ""} ${book.categoryNames.join(" ")} ${book.subjectNames.join(" ")} ${book.shelves.map((item) => `${item.name} ${item.code}`).join(" ")}`.toLowerCase().includes(needle)));
  ...
  const fetchPage = (targetPage: number, limit = safePageSize) => client.rpc("public_catalogue_page_v2", {
    p_library_code: code, p_query: safeQuery, p_available_only: availableOnly, p_limit: limit,
    p_offset: (targetPage - 1) * safePageSize, p_shelf_code: shelfCode.trim() || null, p_book_id: null,
  });
...
export async function getPublicShelves(rawCode: string): Promise<PublicShelf[] | null> {
  const code = normalizeLibraryCode(rawCode);
  if (isLocalDemo(code)) return [{ name: "Biography", code: "B-02", titleCount: 1, copyCount: 5, availableCopyCount: 3 }, { name: "History", code: "H-04", titleCount: 2, copyCount: 6, availableCopyCount: 1 }, { name: "Science", code: "S-11", titleCount: 1, copyCount: 4, availableCopyCount: 0 }];
  if (!getOptionalPublicSupabaseEnv()) return null;
  const { data, error } = await asOperatorRpcClient(await createSupabaseServerClient()).rpc("public_shelf_summaries", { p_library_code: code });
  return error ? null : (data as Array<Record<string, unknown>> ?? []).map((row) => ({ code: String(row.code), name: String(row.name), titleCount: Number(row.title_count), copyCount: Number(row.copy_count), availableCopyCount: Number(row.available_copy_count) }));
}
...
export async function getPublicBook(rawCode: string, id: string): Promise<PublicBook | null> {
  ...
  if (!getOptionalPublicSupabaseEnv()) return null;
  const { data, error } = await asOperatorRpcClient(await createSupabaseServerClient()).rpc("public_catalogue_page_v2", { p_library_code: code, p_query: null, p_available_only: false, p_limit: 1, p_offset: 0, p_shelf_code: null, p_book_id: id });
...
function mapPublicBook(row: Record<string, unknown>): PublicBook {
  return {
    ...
    hasCover: Boolean(row.has_cover), shelves: Array.isArray(row.shelves) ? row.shelves.map((item) => {
      const shelf = item as Record<string, unknown>; return { code: String(shelf.code), name: String(shelf.name), totalCopies: Number(shelf.totalCopies), availableCopies: Number(shelf.availableCopies) };
    }) : [],
  };
}
