...
function isExplicitLocalTestMode() {
  return process.env.SUPABASE_LOCAL_TEST_MODE === "true";
}
...
export function getTrustedAppUrl() {
  ...
  try {
    if (configured) {
      ...
      if (process.env.NODE_ENV === "production" && isLocalhost(new URL(appUrl).hostname) && !isExplicitLocalTestMode()) {
