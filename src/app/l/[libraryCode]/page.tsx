...
export default async function PublicRoomPage({ params }: { params: Promise<{ libraryCode: string }> }) {
  ...
  const [library, books] = await Promise.all([getPublicLibrary(libraryCode), getPublicCatalogue(libraryCode)]);
  ...
  return <>
    ...
    <section className="catalogue-section" aria-labelledby="new-shelves"><header><div><p>Explore</p><h2 id="new-shelves">New to the shelves</h2></div><Link href={`/l/${library.code}/catalogue`}>Full catalogue<ArrowRight aria-hidden="true" /></Link></header>{books === null ? <ReadError /> : catalogue.length ? <div className="book-grid">{catalogue.slice(0, 6).map((book, index) => <BookCard key={book.id} book={book} libraryCode={library.code} priority={index < 3} />)}</div> : <EmptyCatalogue />}</section>
