...
type Query = { error?: string; confirmation?: string; signin?: string; emailConfirmedReturn?: string; name?: string; code?: string; person?: string; currencyCode?: string; localeCode?: string; timeZone?: string };
...
export default async function CreateLibraryPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <main className="onboarding-shell"><header><Brand /><ThemeToggle compact /></header><div className="onboarding-grid"><Intro /><section className="onboarding-form">
    {query.confirmation ? <Verification carry={carry} configured={configured} error={error} /> : query.signin ? <SignIn carry={carry} configured={configured} error={error} emailConfirmedReturn={Boolean(query.emailConfirmedReturn)} /> : verified ? <VerifiedAccount carry={carry} configured={configured} error={error} /> : <Registration carry={carry} configured={configured} error={error} />}
