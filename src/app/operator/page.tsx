...
export default async function OperatorPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <section className="operator-page" aria-labelledby="operator-title">
      ...
      {params.q ? <div className="lookup-list" aria-live="polite">{searchError ? <span className="auth-error" role="alert">Search is unavailable; no conclusion can be drawn from this failed read.</span> : results.length ? results.map((result) => <Link key={`${result.result_type}-${result.result_id}`} href={result.result_type === "book" ? `/operator/catalogue/${result.result_id}` : result.result_type === "member" ? `/operator/members/${result.result_id}` : result.result_type === "loan" ? `/operator/circulation?loan=${encodeURIComponent(result.result_id)}` : result.result_type === "copy" ? `/operator/inventory/${result.result_id}` : "/operator/circulation"}>{result.label} · {result.detail} · {result.status}</Link>) : <span>No matching records.</span>}</div> : null}
