...
export default async function OperatorPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [{ data: dashboard }, { data: searchResults }] = await Promise.all([
  ...
  return (
    <section className="operator-page" aria-labelledby="operator-title">
      ...
      {params.q ? <div className="lookup-list" aria-live="polite">{results.length ? results.map((result) => <Link key={`${result.result_type}-${result.result_id}`} href={result.result_type === "book" ? `/operator/catalogue/${result.result_id}` : result.result_type === "member" ? `/operator/members/${result.result_id}` : "/operator/circulation"}>{result.label} · {result.detail} · {result.status}</Link>) : <span>No matching records.</span>}</div> : null}
