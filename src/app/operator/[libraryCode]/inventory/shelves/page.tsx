...
export default async function ShelvesPage({ params, searchParams }: { params: Promise<{ libraryCode: string }>; searchParams: Promise<{ error?: string; success?: string; edit?: string }> }) {
  ...
  return <section className="operator-page-v3"><OperatorPageHeader title="Shelves" description="Organise physical copies by the shelves, racks, rooms, or sections where they are stored." /><Feedback error={query.error} success={query.success} /><div className="operator-toolbar"><Drawer title={editing ? "Edit shelf" : "Add shelf"} initiallyOpen={Boolean(query.edit)} trigger={<><Plus aria-hidden="true" />{editing ? "Edit shelf" : "Add shelf"}</>}><ShelfForm libraryCode={libraryCode} shelf={editing} /></Drawer><Link className="button button-secondary" href={`/operator/${libraryCode}/inventory`}>Back to inventory</Link></div>{shelves.length ? <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Shelf</th><th>Code</th><th>Titles</th><th>Copies</th><th>Available</th><th>Status</th><th><span className="sr-only">Edit</span></th></tr></thead><tbody>{shelves.map((shelf) => <tr key={shelf.id}><td><strong>{shelf.display_name}</strong></td><td>{shelf.location_code}</td><td>{shelf.title_count}</td><td>{shelf.copy_count}</td><td>{shelf.available_copy_count}</td><td><span className={`status-dot ${shelf.status === "active" ? "status-success" : "status-warning"}`}>{shelf.status}</span></td><td><Drawer title="Edit shelf" trigger={<MoreHorizontal aria-hidden="true" />} triggerClassName="icon-button" triggerAriaLabel={`Edit ${shelf.display_name}`}><ShelfForm libraryCode={libraryCode} shelf={shelf} /></Drawer></td></tr>)}</tbody></table></div> : <div className="empty-state"><h2>No shelves yet</h2><p>Create shelves to help librarians and readers locate physical books.</p></div>}</section>;
}
