...
function done(message: string, data?: unknown): never { redirect(`/operator/circulation?success=${encodeURIComponent(message)}${data ? `&result=${encodeURIComponent(JSON.stringify(data))}` : ""}`); }
...
export async function issueLoanAction(formData: FormData) {
  ...
  try { done("Loan issued", await call("circulation_issue_loan", { p_member_id: memberId, p_book_copy_id: copyId, p_request_id: requestId, p_notes: value(formData, "notes") || null })); } catch (error) { fail(error); }
}
...
export async function returnLoanAction(formData: FormData) {
  ...
  try { done("Loan returned", await call("circulation_return_loan", { p_loan_id: loanId, p_request_id: requestId })); } catch (error) { fail(error); }
}

export async function renewLoanAction(formData: FormData) {
  const loanId = value(formData, "loanId"); const requestId = value(formData, "requestId");
  if (!uuid(loanId) || !uuid(requestId)) fail("invalid input");
  try { done("Loan renewed", await call("circulation_renew_loan", { p_loan_id: loanId, p_request_id: requestId })); } catch (error) { fail(error); }
}

export async function assessFineAction(formData: FormData) {
  const loanId = value(formData, "loanId"); const requestId = value(formData, "requestId");
  if (!uuid(loanId) || !uuid(requestId)) fail("invalid input");
  try { done("Fine assessment checked", await call("circulation_assess_overdue_fine", { p_loan_id: loanId, p_request_id: requestId })); } catch (error) { fail(error); }
}
...
async function amountAction(formData: FormData, name: string, success: string) {
  ...
  try { done(success, await call(name, { p_fine_id: fineId, p_amount_minor: amount, p_request_id: requestId, ...(name.endsWith("settle_fine") ? { p_note: value(formData, "note") || null } : { p_reason: value(formData, "reason") }) })); } catch (error) { fail(error); }
}
