...
export async function GET(_request: Request, { params }: { params: Promise<{ libraryCode: string }> }) {
  ...
  return NextResponse.json(
    library ? { valid: true, code: library.code, name: library.displayName, currencyCode: library.currencyCode, localeCode: library.localeCode, timeZone: library.timeZone } : { valid: false },
